...
const nextConfig = {
  ...
  // Prevent webpack from bundling native Node.js addons
  webpack: (config, { isServer }) => {
    if (isServer) {
      // isolated-vm is a native C++ addon — must be loaded by Node at runtime, not bundled
      config.externals = [
        ...(Array.isArray(config.externals) ? config.externals : [config.externals].filter(Boolean)),
        ({ request }, callback) => {
          if (request === 'isolated-vm') {
